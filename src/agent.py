...
2. 如果某个结论使用了未出现在参考依据中的法律条文，必须在该风险项中标明“补充法条依据”，不得伪装成 RAG 命中依据。
...
7. 三年固定期限劳动合同约定六个月试用期，属于《劳动合同法》第十九条“三年以上固定期限”通常理解范围，不得仅因“三年整”表述判定为风险。
...
2. 检查 Auditor 是否把未出现在参考依据中的法条误写成已检索命中的依据。
3. 检查所设定的高/中/低风险评级是否过激或过宽泛，是否偏离司法实践？
4. 检查没有明确违法事实的合规条款是否被强行标记为风险。
5. 修改对策是否切实可行，是否反而增加了企业或劳动者的其他法律隐患？
6. 特别审查是否把“三年固定期限+六个月试用期”错误认定为风险；如出现，应要求改正。
7. 特别审查是否把已约定但可进一步细化的工作地点、竞业范围等事项过度升格为中风险。
